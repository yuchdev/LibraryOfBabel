...
from babel_poc.cli import app
from babel_poc.vocabulary.manifest import InstalledVocabulary
...
def test_setup_vocab_source_with_mocked_installer(runner: CliRunner) -> None:
    with patch("babel_poc.cli.install_source", return_value=FAKE_ENTRY) as mock_install:
...
def test_setup_vocab_all_with_mocked_installer(runner: CliRunner) -> None:
    with patch("babel_poc.cli.install_all_sources", return_value=[FAKE_ENTRY]) as mock_install:
...
def test_setup_vocab_no_downloadable_sources(runner: CliRunner) -> None:
    with patch("babel_poc.cli.install_all_sources", return_value=[]):
        result = runner.invoke(app, ["setup-vocab", "--all"])
    assert result.exit_code == 0
...
def test_page_without_vocab_finds_installed(
    ...
    import babel_poc.vocabulary.discovery as disc
